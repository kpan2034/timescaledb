			...
			if (ts_chunk_contains_compressed_data(chunk))
				ereport(ERROR,
						(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						 ...
						 errhint(
							 "Please drop the corresponding chunk on the rowstore hypertable "
							 "instead.")));
						...
						ereport(ERROR,
								(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
								 errmsg(
									 ...
									 "data"),
								 errhint("Convert the data to rowstore before retrying the operation.")));
			}
			break;
...
process_altertable_set_options(AlterTableCmd *cmd, Hypertable *ht)
{
	...
	if (pg_options != NIL)
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg("only timescaledb.enable_columnstore parameters allowed when specifying columnstore "
