...
compresschunkcxt_init(CompressChunkCxt *cxt, Cache *hcache, Oid hypertable_relid, Oid chunk_relid)
{
	...
	if (compress_ht == NULL)
		ereport(ERROR, (errcode(ERRCODE_INTERNAL_ERROR), errmsg("missing columnstore hypertable")));
...
decompress_chunk_impl(Chunk *uncompressed_chunk, bool if_compressed)
{
	...
	if (TS_HYPERTABLE_IS_INTERNAL_COMPRESSION_TABLE(uncompressed_hypertable))
		ereport(ERROR,
				(errcode(ERRCODE_INTERNAL_ERROR),
				 errmsg("convert_to_rowstore must not be called on the internal columnstore chunk")));
				   ...
				   bool if_not_compressed, bool recompress)
{
	...
	switch (useam)
	{
		case USE_AM_FALSE:
			...
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					 errmsg("cannot converting to columnstore \"%s\" without using Hypercore access method",
							get_rel_name(chunk->table_id)),
					 errhint(
						 "Convert to rowstore first and then convert to columnstore without Hypercore access method.")));
			break;
		case USE_AM_NULL:
			Assert(rel_is_hypercore);
		...
		if (!ts_chunk_needs_recompression(chunk))
		{
			...
			ereport((if_not_compressed ? NOTICE : ERROR),
					(errcode(ERRCODE_DUPLICATE_OBJECT),
					 errmsg("chunk \"%s\" is already converted to columnstore", get_rel_name(chunk->table_id))));
			return uncompressed_chunk_id;
		}
...
tsl_decompress_chunk(PG_FUNCTION_ARGS)
{
	...
	else if (!ts_chunk_is_compressed(uncompressed_chunk))
	{
		ereport((if_compressed ? NOTICE : ERROR),
				(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
				 errmsg("chunk \"%s\" is not converted to columnstore", get_rel_name(uncompressed_chunk_id))));
